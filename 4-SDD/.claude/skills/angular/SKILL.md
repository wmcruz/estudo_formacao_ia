---
name: angular
description: Use when writing or reviewing Angular code — standalone components, component class limits (≤30 lines), explicit @Input/@Output, DI with inject(), avoiding method calls in templates (use computed() or pure pipes), and Angular-specific file naming conventions.
---

<critical>
Sempre que esta skill for carregada, você DEVE carregar e processar TODAS AS REFERENCES criadas no contexto da conversa. Não ignore nenhuma referência, arquivo ou instrução adicional fornecida pelo usuário.
</critical>
